...
export default function AppointmentDetailsScreen() {
  ...
  const loadAppointment = async () => {
    try {
      ...
      const apt = response.data?.appointments?.find(
      ...
      <View className="px-4 pt-4 pb-8">
        ...
        {appointment.status === 'booked' && (
          <View className="gap-3">
            ...
            >
              <Text className="text-white font-semibold">Start Appointment</Text>
            </TouchableOpacity>
            <TouchableOpacity
          ...
          <View className="gap-3">
            ...
            {appointment.staff?.id && (
              <View className="bg-primary-50 rounded-lg p-4 border border-primary-200">
                <Text className="text-text-secondary text-sm mb-2">Your Rating</Text>
                <View className="flex-row items-center">
                  <Ionicons name="star" size={20} color="#f59e0b" />
                  <Text className="text-text-primary font-semibold ml-2">
                    Check your performance screen for ratings
                  </Text>
                </View>
              </View>
            )}
